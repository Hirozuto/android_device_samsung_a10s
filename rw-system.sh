...
if [ -z "$debug" ] && [ -f /cache/a10s-log ];then
	mkdir -p /cache/a10s
	debug=1 exec sh -x "$(readlink -f -- "$0")" > /cache/a10s/logs 2>&1
else
    # Allow accessing logs from system app
    # Protected via SELinux for other apps
    chmod 0755 /cache/a10s
    chmod 0644 /cache/a10s/logs
fi

if [ -f /cache/a10s-adb ];then
    ...
    echo a10s > /config/usb_gadget/g1/strings/0x409/serialnumber
    echo a10s > /config/usb_gadget/g1/strings/0x409/manufacturer
    echo a10s > /config/usb_gadget/g1/strings/0x409/product
...
if [ "$vndk" = 26 ];then
	resetprop_a10s ro.vndk.version 26
fi
...
fixSPL() {
    ...
    if [ -f /vendor/bin/hw/android.hardware.keymaster@4.1-service.trustkernel ] && [ -f /proc/tkcore/tkcore_log ];then
        setprop debug.a10s.props.teed keymaster
        # Process name is android.hardware.keymaster@4.1-service.trustkernel
        setprop debug.a10s.props.ice.trustkernel keymaster
    fi
        ...
        /vendor/lib/hw/libMcTeeKeymaster.so /vendor/lib64/hw/libMcTeeKeymaster.so $additional; do
            ...
            sed -i \
                ...
                "/mnt/a10s/$b"
            chcon "$ctxt" "/mnt/a10s/$b"
            mount -o bind "/mnt/a10s/$b" "$f"
        done
...
changeKeylayout() {
    mpk="/mnt/a10s/keylayout"
    cp -a /system/usr/keylayout /mnt/a10s/keylayout
    changed=false
    if grep -q vendor.huawei.hardware.biometrics.fingerprint /vendor/etc/vintf/manifest.xml; then
        changed=true
        cp /system/a10s/huawei/fingerprint.kl /mnt/a10s/keylayout/fingerprint.kl
        chmod 0644 /mnt/a10s/keylayout/fingerprint.kl
    fi
    ...
    if getprop ro.vendor.build.fingerprint |
        ...
        cp /system/a10s/samsung-gpio_keys.kl /mnt/a10s/keylayout/gpio_keys.kl
        cp /system/a10s/samsung-sec_touchscreen.kl /mnt/a10s/keylayout/sec_touchscreen.kl
        cp /system/a10s/samsung-sec_touchkey.kl /mnt/a10s/keylayout/sec_touchkey.kl
        chmod 0644 /mnt/a10s/keylayout/gpio_keys.kl /mnt/a10s/keylayout/sec_touchscreen.kl
    fi

    if getprop ro.vendor.build.fingerprint | grep -iq \
        -e poco/ -e POCO/ -e redmi/ -e xiaomi/ ; then
        if [ ! -f /mnt/a10s/keylayout/uinput-goodix.kl ]; then
          cp /system/a10s/empty /mnt/a10s/keylayout/uinput-goodix.kl
          chmod 0644 /mnt/a10s/keylayout/uinput-goodix.kl
          changed=true
        fi
        if [ ! -f /mnt/a10s/keylayout/uinput-fpc.kl ]; then
          cp /system/a10s/empty /mnt/a10s/keylayout/uinput-fpc.kl
          chmod 0644 /mnt/a10s/keylayout/uinput-fpc.kl
          changed=true
        fi
    fi

    if getprop ro.vendor.build.fingerprint | grep -iq -e xiaomi/daisy; then
        cp /system/a10s/daisy-buttonJack.kl ${mpk}/msm8953-snd-card-mtp_Button_Jack.kl
        changed=true
        if [ ! -f /mnt/a10s/keylayout/uinput-goodix.kl ]; then
           cp /system/a10s/daisy-uinput-goodix.kl ${mpk}/uinput-goodix.kl
           changed=true
        fi
        if [ ! -f /mnt/a10s/keylayout/uinput-fpc.kl ]; then
           cp /system/a10s/daisy-uinput-fpc.kl ${mpk}/uinput-fpc.kl
           changed=true
        fi
    ...
    if getprop ro.vendor.build.fingerprint | grep -iq -e xiaomi/renoir; then
        mpk="/mnt/a10s/keylayout"
        cp /system/a10s/daisy-buttonJack.kl ${mpk}/lahaina-shimaidp-snd-card_Button_Jack.kl
    ...
    if getprop ro.vendor.build.fingerprint | grep -qi oneplus/oneplus6/oneplus6; then
        cp /system/a10s/oneplus6-synaptics_s3320.kl /mnt/a10s/keylayout/synaptics_s3320.kl
        chmod 0644 /mnt/a10s/keylayout/synaptics_s3320.kl
        changed=true
    fi

    if getprop ro.vendor.build.fingerprint | grep -iq -e iaomi/perseus -e iaomi/cepheus; then
        cp /system/a10s/mimix3-gpio-keys.kl /mnt/a10s/keylayout/gpio-keys.kl
        chmod 0644 /mnt/a10s/keylayout/gpio-keys.kl
        changed=true
    fi

    if getprop ro.vendor.build.fingerprint | grep -iq -E -e '^Sony/'; then
        cp /system/a10s/sony-gpio-keys.kl /mnt/a10s/keylayout/gpio-keys.kl
        chmod 0644 /mnt/a10s/keylayout/gpio-keys.kl
        changed=true
    fi

    if getprop ro.vendor.build.fingerprint |grep -iq -E -e '^Nokia/Panther';then
        cp /system/a10s/nokia-soc_gpio_keys.kl /mnt/a10s/keylayout/soc_gpio_keys.kl
        chmod 0644 /mnt/a10s/keylayout/soc_gpio_keys.kl
        changed=true
    fi

    if getprop ro.vendor.build.fingerprint |grep -iq -E -e '^Lenovo/' && [ -f /sys/devices/virtual/touch/tp_dev/gesture_on ];then
        cp /system/a10s/lenovo-synaptics_dsx.kl /mnt/a10s/keylayout/synaptics_dsx.kl
        chmod 0644 /mnt/a10s/keylayout/synaptics_dsx.kl
        cp /system/a10s/lenovo-synaptics_dsx.kl /mnt/a10s/keylayout/fts_ts.kl
        chmod 0644 /mnt/a10s/keylayout/fts_ts.kl
        changed=true
    fi
	...
	echo 1 > /proc/touchpanel/double_tap_enable
        cp /system/a10s/oppo-touchpanel.kl /mnt/a10s/keylayout/touchpanel.kl
	cp /system/a10s/oppo-touchpanel.kl /mnt/a10s/keylayout/mtk-tpd.kl
        chmod 0644 /mnt/a10s/keylayout/touchpanel.kl
	chmod 0644 /mnt/a10s/keylayout/mtk-tpd.kl
        changed=true
    fi

    if getprop ro.vendor.build.fingerprint |grep -q -e google/;then
        cp /system/a10s/google-uinput-fpc.kl /mnt/a10s/keylayout/uinput-fpc.kl
        chmod 0644 /mnt/a10s/keylayout/uinput-fpc.kl
        changed=true
    fi

    if getprop ro.product.vendor.manufacturer |grep -q -e motorola;then
        cp /system/a10s/moto-uinput-egis.kl /mnt/a10s/keylayout/uinput-egis.kl
        cp /system/a10s/moto-uinput-egis.kl /mnt/a10s/keylayout/uinput-fpc.kl
        chmod 0644 /mnt/a10s/keylayout/uinput-egis.kl
        chmod 0644 /mnt/a10s/keylayout/uinput-fpc.kl
        changed=true
    fi

    if getprop ro.vendor.build.fingerprint |grep -q -e nubia/NX659;then
        cp /system/a10s/nubia-nubia_synaptics_dsx.kl /mnt/a10s/keylayout/nubia_synaptics_dsx.kl
        chmod 0644 /mnt/a10s/keylayout/nubia_synaptics_dsx.kl
        changed=true
    fi

    if getprop ro.vendor.build.fingerprint |grep -q -i -e Teracube/Teracube_2e;then
        cp /system/a10s/teracube2e-mtk-kpd.kl /mnt/a10s/keylayout/mtk-kpd.kl
        chmod 0644 /mnt/a10s/keylayout/mtk-kpd.kl
        changed=true
    fi

    if getprop ro.vendor.asus.build.fp |grep -q ASUS_I01WD;then
        cp /system/a10s/zf6-goodixfp.kl /mnt/a10s/keylayout/goodixfp.kl
        cp /system/a10s/zf6-googlekey_input.kl /mnt/a10s/keylayout/googlekey_input.kl
        chmod 0644 /mnt/a10s/keylayout/goodixfp.kl
        chmod 0644 /mnt/a10s/keylayout/googlekey_input.kl
        changed=true
    fi

    if getprop ro.vendor.build.fingerprint | grep -q -e Unihertz/;then
        cp /system/a10s/unihertz-mtk-kpd.kl /mnt/a10s/keylayout/mtk-kpd.kl
        cp /system/a10s/unihertz-mtk-tpd.kl /mnt/a10s/keylayout/mtk-tpd.kl
        cp /system/a10s/unihertz-mtk-tpd-kpd.kl /mnt/a10s/keylayout/mtk-tpd-kpd.kl
        cp /system/a10s/unihertz-fingerprint_key.kl /mnt/a10s/keylayout/fingerprint_key.kl
        chmod 0644 /mnt/a10s/keylayout/mtk-kpd.kl
        chmod 0644 /mnt/a10s/keylayout/mtk-tpd.kl
        chmod 0644 /mnt/a10s/keylayout/mtk-tpd-kpd.kl
        chmod 0644 /mnt/a10s/keylayout/fingerprint_key.kl
        changed=true
    fi

    if getprop ro.vendor.build.fingerprint | grep -q -i -e Blackview/BV9500Plus;then
        cp /system/a10s/bv9500plus-mtk-kpd.kl /mnt/a10s/keylayout/mtk-kpd.kl
        chmod 0644 /mnt/a10s/keylayout/mtk-kpd.kl
        changed=true
    fi
    ...
    if getprop ro.vendor.build.fingerprint | grep -iq -e motorola/liber; then
        cp /system/a10s/moto-liber-gpio-keys.kl /mnt/a10s/keylayout/gpio-keys.kl
        chmod 0644 /mnt/a10s/keylayout/gpio-keys.kl

        cp /system/a10s/empty /mnt/a10s/keylayout/uinput_nav.kl
        chmod 0644 /mnt/a10s/keylayout/uinput_nav.kl
        changed=true
    fi

    if getprop ro.vendor.build.fingerprint | grep -iq DOOGEE/S88Pro;then
          cp /system/a10s/empty /mnt/a10s/keylayout/sf-keys.kl
          chmod 0644 /mnt/a10s/keylayout/sf-keys.kl
          changed=true
    fi
    ...
    if getprop ro.vendor.build.fingerprint |grep -iq tecno/kd7;then
        ...
        cp /system/a10s/tecno-touchpanel.kl /mnt/a10s/keylayout/mtk-tpd.kl
        chmod 0644 /mnt/a10s/keylayout/mtk-tpd.kl
        changed=true
    fi

    if [ "$changed" = true ]; then
        mount -o bind /mnt/a10s/keylayout /system/usr/keylayout
...
if [ "$(getprop ro.product.vendor.manufacturer)" = motorola ] && getprop ro.vendor.product.name |grep -qE '^lima';then
    for l in lib lib64;do
        for f in mt6771 lima;do
            mount /system/a10s/empty /vendor/$l/hw/keystore.$f.so
        done
    done
...
if mount -o remount,rw /system; then
    ...
    mknod /dev/tmp-a10s b $((0x$major)) $((0x$minor))
    blockdev --setrw /dev/tmp-a10s
    resize2fs /dev/root || true
    resize2fs /dev/tmp-a10s || true
fi
...
mkdir -p /mnt/a10s/
mount -t tmpfs -o rw,nodev,relatime,mode=755,gid=0 none /mnt/a10s || true
mkdir /mnt/a10s/empty_dir
fixSPL

changeKeylayout

mount /system/a10s/empty /vendor/bin/vendor.samsung.security.proca@1.0-service || true

if grep vendor.huawei.hardware.biometrics.fingerprint /vendor/manifest.xml; then
    mount -o bind system/a10s/huawei/fingerprint.kl /vendor/usr/keylayout/fingerprint.kl
fi
...
if [ "$foundFingerprint" = false ];then
    mount -o bind system/a10s/empty /system/etc/permissions/android.hardware.fingerprint.xml
fi
...
if ! grep android.hardware.bluetooth /vendor/manifest.xml && ! grep android.hardware.bluetooth /vendor/etc/vintf/manifest.xml; then
    mount -o bind system/a10s/empty /system/etc/permissions/android.hardware.bluetooth.xml
    mount -o bind system/a10s/empty /system/etc/permissions/android.hardware.bluetooth_le.xml
fi
...
if getprop ro.vendor.product.device |grep -iq -e RMX2001 -e RMX2151 -e RMX2111 -e RMX2111L1;then
    setprop persist.sys.a10s.fingerprint.nocleanup true
    setprop persist.sys.qcom-brightness "$(cat /sys/class/leds/lcd-backlight/max_brightness)"
fi
...
if getprop ro.vendor.build.fingerprint | grep -iq \
    ...
    mount -o bind /mnt/a10s/empty_dir /vendor/lib64/soundfx
    mount -o bind /mnt/a10s/empty_dir /vendor/lib/soundfx
...
for f in /vendor/lib/mtk-ril.so /vendor/lib64/mtk-ril.so /vendor/lib/libmtk-ril.so /vendor/lib64/libmtk-ril.so; do
    ...
    setprop persist.sys.a10s.radio.force_cognitive true
...
mount -o bind /system/a10s/empty /vendor/lib/libpdx_default_transport.so
mount -o bind /system/a10s/empty /vendor/lib64/libpdx_default_transport.so

mount -o bind /system/a10s/empty /vendor/overlay/SysuiDarkTheme/SysuiDarkTheme.apk || true
mount -o bind /system/a10s/empty /vendor/overlay/SysuiDarkTheme/SysuiDarkThemeOverlay.apk || true
...
if [ -f /vendor/bin/hw/vendor.samsung.hardware.miscpower@1.0-service ] && [ "$vndk" -lt 28 ]; then
    mount -o bind /system/a10s/empty /vendor/bin/hw/android.hardware.power@1.0-service
fi

if [ "$vndk" = 27 ] || [ "$vndk" = 26 ]; then
    mount -o bind /system/a10s/libnfc-nci-oreo.conf /system/etc/libnfc-nci.conf
fi

if busybox_a10s unzip -p /vendor/app/ims/ims.apk classes.dex | grep -qF -e Landroid/telephony/ims/feature/MmTelFeature -e Landroid/telephony/ims/feature/MMTelFeature; then
    mount -o bind /system/a10s/empty /vendor/app/ims/ims.apk
fi
...
if getprop ro.vendor.build.fingerprint | grep -iq -e Redmi/rosemary \
    ...
    mount -o bind /system/a10s/rosemary-excluded-input-devices.xml /system/etc/excluded-input-devices.xml
fi
...
if getprop ro.vendor.build.fingerprint | grep -iq -E -e 'huawei|honor' || getprop persist.sys.overlay.huawei | grep -iq -E -e 'true'; then
    ...
    mount -o bind /system/a10s/libnfc-nci-huawei.conf /system/etc/libnfc-nci.conf
fi
...
if getprop ro.vendor.build.fingerprint |grep -qE razer/cheryl;then
	setprop ro.audio.monitorRotation true
	mount -o bind /system/a10s/empty /vendor/overlay/BluetoothResCommon.apk
	mount -o bind /system/a10s/empty /vendor/overlay/RazerCherylBluetoothRes.apk
fi
...
if getprop ro.vendor.build.fingerprint | grep -qiE '^samsung'; then
    ...
    if getprop ro.hardware | grep -q -e samsungexynos7870 -e qcom; then
        if [ "$vndk" -le 27 ]; then
            setprop persist.sys.a10s.sdk_override /vendor/bin/hw/rild=27
        fi
    fi
fi
...
if [ $(find /vendor/etc/audio -type f |wc -l) -le 3 ];then
	mount -o bind /mnt/a10s/empty_dir /vendor/etc/audio || true
fi
...
for f in /vendor/lib{,64}/hw/com.qti.chi.override.so /vendor/lib{,64}/libVD*;do
    ...
    cp -a "$f" "/mnt/a10s/$b"
    sed -i \
        -e 's/ro.product.manufacturer/sys.a10s.xx.manufacturer/g' \
        -e 's/ro.product.brand/sys.a10s.xx.brand/g' \
        -e 's/ro.product.model/sys.a10s.xx.model/g' \
        "/mnt/a10s/$b"
    chcon "$ctxt" "/mnt/a10s/$b"
    mount -o bind "/mnt/a10s/$b" "$f"
    ...
    setprop sys.a10s.xx.manufacturer "$manufacturer"
    setprop sys.a10s.xx.brand "$(getprop ro.product.vendor.brand)"
    setprop sys.a10s.xx.model "$model"
done
...
if getprop ro.vendor.build.fingerprint | grep -qiE '^samsung/' && [ "$vndk" -ge 28 ];then
	setprop persist.sys.a10s.samsung_fingerprint 0
...
copyprop() {
    p="$(getprop "$2")"
    if [ "$p" ]; then
        resetprop_a10s "$1" "$(getprop "$2")"
    fi
}
if [ -f /system/a10s/secure ] || [ -f /metadata/a10s/secure ];then
    ...
    (getprop ro.vendor.build.security_patch; getprop ro.keymaster.xxx.security_patch) |sort |tail -n 1 |while read v;do
        [ -n "$v" ] && resetprop_a10s ro.build.version.security_patch "$v"
    done

    resetprop_a10s ro.build.tags release-keys
    resetprop_a10s ro.boot.vbmeta.device_state locked
    resetprop_a10s ro.boot.verifiedbootstate green
    resetprop_a10s ro.boot.flash.locked 1
    resetprop_a10s ro.boot.veritymode enforcing
    resetprop_a10s ro.boot.warranty_bit 0
    resetprop_a10s ro.warranty_bit 0
    resetprop_a10s ro.debuggable 0
    resetprop_a10s ro.secure 1
    resetprop_a10s ro.build.type user
    resetprop_a10s ro.build.selinux 0

    resetprop_a10s ro.adb.secure 1
    setprop ctl.restart adbd

    # Hide system/xbin/su
    mount /mnt/a10s/empty_dir /system/xbin
    mount /mnt/a10s/empty_dir /system/app/me.a10s.superuser
    mount /system/a10s/empty /system/xbin/a10s-su
else
    mkdir /mnt/a10s/xbin
    chmod 0755 /mnt/a10s/xbin
    chcon u:object_r:system_file:s0 /mnt/a10s/xbin

    #a10s-su will bind over this empty file to make a real su
    touch /mnt/a10s/xbin/su
    chcon u:object_r:system_file:s0 /mnt/a10s/xbin/su

    mount -o bind /mnt/a10s/xbin /system/xbin
fi
...
if [ "$has_hostapd" = false ];then
    setprop persist.sys.a10s.system_hostapd true
fi
...
if getprop ro.hardware.keystore | grep -iq sprd && [ "$vndk" -le 28 ]; then
    setprop persist.sys.a10s.disable_sensor_direct_report true
fi
...
if getprop ro.vendor.build.fingerprint | grep -iq \
    ...
    setprop persist.sys.a10s.radio.use_old_mnc_format true
fi

if getprop ro.build.overlay.deviceid |grep -qE '^RMX';then
    resetprop_a10s ro.vendor.gsi.build.flavor byPass
    setprop oppo.camera.packname com.oppo.engineermode.camera
    setprop sys.a10s.xx.brand realme
fi
...
resetprop_a10s service.adb.root 0
...
if getprop ro.vendor.build.fingerprint |grep -qiE '^xiaomi/';then
    setprop persist.sys.a10s.fod.xiaomi true
fi
...
resetprop_a10s ro.bluetooth.library_name libbluetooth.so
...
if [ "$board" = lahaina ]; then
	...
	resetprop_a10s ro.config.media_vol_steps 25
	resetprop_a10s ro.config.media_vol_default 15
fi
...
if getprop ro.bionic.cpu_variant |grep -q kryo300;then
    resetprop_a10s ro.bionic.cpu_variant cortex-a75
    setprop dalvik.vm.isa.arm64.variant cortex-a75
    setprop dalvik.vm.isa.arm64.features runtime
fi

resetprop_a10s ro.control_privapp_permissions log
...
if [ "$vndk" -le 27 ];then
    setprop persist.sys.a10s.no_present_or_validate true
fi

[ -d /mnt/vendor/persist ] && mount /mnt/vendor/persist /persist

for f in $(find /sys -name fts_gesture_mode);do
    setprop persist.sys.a10s.focaltech_node "$f"
done

if [ "$vndk" -le 27 ] && [ -f /vendor/bin/mnld ];then
    setprop persist.sys.a10s.sdk_override /vendor/bin/mnld=26
fi
...
if getprop ro.vendor.build.fingerprint | grep -iq -e GOME/GOME_U9; then
    cp -a /system/etc/smartpa_params /mnt/a10s/smartpa_params
    cp /system/a10s/gome/fs16xx_01s_left.preset /mnt/a10s/smartpa_params/fs16xx_01s_left.preset
    cp /system/a10s/gome/fs16xx_01s_mono.preset /mnt/a10s/smartpa_params/fs16xx_01s_mono.preset
    cp /system/a10s/gome/fs16xx_01s_right.preset /mnt/a10s/smartpa_params/fs16xx_01s_right.preset
    chmod 0644 /mnt/a10s/smartpa_params/fs16xx_01s_left.preset
    chmod 0644 /mnt/a10s/smartpa_params/fs16xx_01s_mono.preset
    chmod 0644 /mnt/a10s/smartpa_params/fs16xx_01s_right.preset
    mount -o bind /mnt/a10s/smartpa_params /system/etc/smartpa_params
    restorecon -R /system/etc/smartpa_params
fi

if getprop ro.vendor.build.fingerprint | grep -iq -e UMIDIGI/UMIDIGI_X; then
    cp -a /system/etc/smartpa_params /mnt/a10s/smartpa_params
    cp /system/a10s/umidigi/fs16xx_01s_mono.preset /mnt/a10s/smartpa_params/fs16xx_01s_mono.preset
    chmod 0644 /mnt/a10s/smartpa_params/fs16xx_01s_mono.preset
    mount -o bind /mnt/a10s/smartpa_params /system/etc/smartpa_params
    restorecon -R /system/etc/smartpa_params
fi
...
if [ -f /vendor/bin/ccci_rpcd ];then
    setprop debug.a10s.props.ccci_rpcd vendor
fi
...
if getprop ro.vendor.build.fingerprint | grep -iq -e motorola/liber; then
  cp /vendor/etc/audio_policy_configuration.xml /mnt/a10s/
  sed -i '/r_submix_audio_policy_configuration/a \t<xi:include href="/vendor/etc/a2dp_audio_policy_configuration.xml"/>' /mnt/a10s/audio_policy_configuration.xml
  mount -o bind /mnt/a10s/audio_policy_configuration.xml /vendor/etc/audio_policy_configuration.xml
...
mount /system/a10s/empty /vendor/etc/permissions/samsung.hardware.uwb.xml
mount /system/a10s/empty /vendor/bin/install-recovery.sh
...
mount -o bind /mnt/a10s/empty_dir /vendor/app/qti-logkit
mount -o bind /mnt/a10s/empty_dir /vendor/app/qti-logkit-lite
